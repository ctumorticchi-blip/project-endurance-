...
function fullAvailability(restDays: Availability['restDays'] = ['sunday']): Availability {
  ...
  for (const day of ['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday'] as const) {
    pattern[day] = { available: true, minutes: 60, poolAccess: false }
  }
  return { weeklyPattern: pattern, exceptions: [], restDays }
}

describe('regeneratePlanFromToday (running)', () => {
  it('keeps every week strictly before the current one untouched', () => {
    ...
    const { plan: originalPlan } = generateRunningPlan({ raceGoal, availability: fullAvailability(), today })
  ...
  it('applies the new availability from the current week onward', () => {
    ...
    const raceGoal = createRaceGoal({ sport: 'running', distance: '10k', raceDate: '2026-03-30' })
    const { plan: originalPlan } = generateRunningPlan({
      raceGoal,
      availability: fullAvailability(['sunday']),
      today,
    })
